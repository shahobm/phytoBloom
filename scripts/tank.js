  ...
  class AnalogTank {
    constructor($this, config) {
      let defaults = {
        ...
        backFillColor: "#ffff80", // background color inside the tank where there is no water.
        ...
        tooltipFontSize: 10,
        thresholds: [],
        lightValue: null, // if lookup table value is set, a secondary text is displayed under the main text.
        lightValueUnit: null, // unit for lightValue.
        lightValueDecimal: 0, // number of decimal places for lookup table value.
...
*/
    ...
    initRound () {
      ...
      if (this.lightValueEnabled) {
        this.lightValueBehindText = this.tankGroup.append('text').attr('id', 'light-behind-text');
      }

      if (this.changeDateValueEnabled) {
        this.changeDateValueBehindText = this.tankGroup.append('text').attr('id', 'light-value-rate-behind-text');
      }
      ...
      if (this.changeDateValueEnabled) {
        this.changeDateValueOverlayText = this.waveGroup.append('text').attr('id', 'lookup-value-rate-overlay-text');
      }
    ...
    updateFillColor (options) {
      if ( typeof options === "object" ) {
        if ( "backFillColor" in options ) {
          this.fillColor = options.backFillColor;
        }
